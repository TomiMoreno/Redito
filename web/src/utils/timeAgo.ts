const DATE_UNITS: dateObject = new Map([
  ["day", 86400],
  ["hour", 3600],
  ["minute", 60],
  ["second", 1],
]);
type dateObject =  Map<Intl.RelativeTimeFormatUnit, number>;

const getSecondsDiff = (timestamp: number) => (Date.now() - timestamp) / 1000;
const getUnitAndValueDate = (secondsElapsed: number):{value:number, unit:Intl.RelativeTimeFormatUnit} => {
  const DateUnits = DATE_UNITS.entries()
  for (const [unit, secondsInUnit] of DateUnits) {
    if (secondsElapsed >= secondsInUnit || unit === 'second') {
      const value = Math.floor(secondsElapsed / secondsInUnit) * -1;
      return { value, unit};
    }
  }
  const value = Math.floor(secondsElapsed / 1) * -1;
  return { value, unit: 'second' };
};

export const getTimeAgo = (timestamp: number) => {
  const rtf = new Intl.RelativeTimeFormat();

  const secondsElapsed = getSecondsDiff(timestamp);
  const { value, unit } = getUnitAndValueDate(secondsElapsed);
  return rtf.format(value, unit);
};
